...
*/
function anchorLiquidationApi(mkstr,lcdurl='https://lcd.terra.dev'){
...
async function get_bids(collateral_asset = "bluna",pending_bids = false) {
    ...
    return new Promise((resolve, reject) => {
      request(anchor_bids_query_url, (error, response, body) => {
        ...
        while(x < bids_data.length){
          ...
          if((pending_bids ==true) && (bid_wait_end !== null)){
            ...
            if ((bid_wait_end == null) && (pending_bids !==true)){
              bids.push(bid_id)
            }
          }
...
async function activate_pending_bids(asset){ 
  ...
  console.log("Error: no outstanding bids to activate")
  }
}
this.activatePendingBids = activate_pending_bids

async function cancel_open_bids(asset){ 
  response = await get_bids(asset,false)
  x=0
  while(x < response.length){ 
    bidId = response[x]
    console.log("Cancelling bid id: " +bidId)
    await withdraw_bid(bidId)
    await sleep (1000) //slight delay incase were processing multiple transactions 
    x = x+1
  }
...
module.exports.anchorLiquidationApi = anchorLiquidationApi